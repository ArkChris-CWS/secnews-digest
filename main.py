#!/usr/bin/env python3
"""
보안 아침 브리핑 봇 (가독성 개선본 v3)
- 수집 / 본문 fetch / CISA KEV: 기존과 동일
- [변경] Gemini가 '구조화 JSON'으로 요약 → 파이썬이 텔레그램 HTML로 포맷
- [변경] 라벨을 볼드+이모지로, 항목 구분선/번호 추가 → 가독성 향상
- [핵심] AT&T 의 &, 버전 < 등 특수문자는 html.escape로 안전 처리 (HTML 파싱 깨짐 방지)
- [안전] JSON 파싱 실패 시 평문 폴백 → 절대 끊기지 않음
...
GEMINI_MODEL = "gemini-2.5-flash"   # 무료 티어. 모델명은 Google AI Studio에서 확인
...
[출력 형식 — 매우 중요]
- 반드시 아래 구조의 JSON만 출력한다. 코드펜스(```)나 설명 문장을 붙이지 마라.
...
def summarize(items):
    ...
    body = {
        "contents": [{"parts": [{"text": PROMPT + payload}]}],
        # Gemini에게 JSON으로만 답하도록 강제(지원 모델에서 안정성 ↑)
...
def main():
    ...
    if not data or "items" not in data:
        # JSON 파싱 실패 시 안전 폴백: 평문 전송(절대 끊기지 않음)
        print("JSON 파싱 실패 → 평문 폴백")
